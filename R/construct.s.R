...
, "logl"= gamma.vec <- c(exp(coef(extrapolation)[1,1]+coef(extrapolation)[1,2]*lambda[1]),exp(coef(extrapolation)[1,1]+coef(extrapolation)[2,1]*lambda[1])*lambda[1])
)
a <- null.mat
a[,j] <- gamma.vec # exchange the column j with the deviation of G(gamma, lambda)
for(i in 2:nl){
switch(fitting.method,
